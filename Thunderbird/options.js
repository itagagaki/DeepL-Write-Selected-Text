...
function restoreOptions() {
  browser.storage.local.get(['target'], result => {
    const target = result.target ?? 'tab';
    Array.from(document.getElementsByName('target')).forEach(elem => { elem.checked = elem.value == target; });
  });
}
...
document.querySelectorAll('input').forEach(elem => {
  elem.addEventListener("change", e => updateOptions(e));
});
